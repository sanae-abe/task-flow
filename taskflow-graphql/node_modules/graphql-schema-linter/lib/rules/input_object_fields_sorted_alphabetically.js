...
function InputObjectFieldsSortedAlphabetically(configuration, context) {
  ...
  return {
    InputObjectTypeDefinition(node) {
      ...
      const {
        isSorted,
        sortedList
      } = (0, _listIsAlphabetical.default)(fieldList, sortOrder);

      if (!isSorted) {
        context.reportError(new _validation_error.ValidationError(ruleKey, `The fields of input type \`${node.name.value}\` should be sorted in ${sortOrder} order. ` + `Expected sorting: ${sortedList.join(', ')}`, [node]));
      }
    }
