...
export class PathValidator {
    ...
    validate(filePath) {
        ...
        if (!resolvedPath.startsWith(this.allowedBasePath)) {
            throw new Error(`Path traversal detected: ${filePath} resolves outside allowed base path`);
        }
        // Windowsの代替データストリーム対策
        if (process.platform === 'win32' && resolvedPath.includes(':')) {
            const colonCount = (resolvedPath.match(/:/g) || []).length;
            if (colonCount > 1) {
                throw new Error('Invalid file path: alternate data streams not allowed');
            }
        }
        return resolvedPath;
    }
    ...
    async validateAsync(filePath) {
        ...
        try {
            ...
            if (!realPath.startsWith(allowedRealPath)) {
                throw new Error(`Symbolic link traversal detected: ${filePath} resolves to ${realPath} which is outside allowed base path`);
            }
            return realPath;
        }
        catch (error) {
     ...
     */
    async validateFileSize(filePath, maxSizeMB = 5) {
        const stats = await fs.stat(filePath);
        const maxBytes = maxSizeMB * 1024 * 1024;
        if (stats.size > maxBytes) {
            throw new Error(`File size exceeds ${maxSizeMB}MB limit: ${Math.round((stats.size / 1024 / 1024) * 100) / 100}MB`);
        }
    }
    /**
     * ファイルが存在することを確認します
