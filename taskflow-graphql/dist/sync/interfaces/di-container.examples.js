...
export function example2_Transient() {
    ...
    // 解決（異なるインスタンスが返される）
    const limiter1 = container.resolve('rateLimiter');
    const limiter2 = container.resolve('rateLimiter');
...
export function example3_Factory() {
    ...
    // ファクトリー解決
    const fs1 = container.resolveFactory('fileSystem', '/path/to/workspace');
    const fs2 = container.resolveFactory('fileSystem', '/different/path');
...
export function example5_DependencyChain() {
    ...
    container.registerSingleton('fileSystem', () => {
...
export function example7_ChildContainer() {
    ...
    const logger = child.resolve('logger'); // 親から解決
    const limiter = child.resolve('rateLimiter'); // 子から解決
...
export function example8_Diagnostics() {
    ...
    container.registerSingleton('logger', () => Logger.getInstance());
    container.registerTransient('rateLimiter', () => new RateLimiter());
    container.registerFactory('fileSystem', (path) => new RealFileSystem(path));
    // いくつか解決してキャッシュを作成
...
export function example9_RealWorldSetup() {
    ...
    container.registerSingleton('syncService', () => {
        ...
        return {
            sync: async (path) => {
                const fs = container.resolveFactory('fileSystem', path);
...
export function example11_MethodChaining() {
    const container = new DIContainer()
        ...
        .registerFactory('fileSystem', (path) => new RealFileSystem(path));
...
export function example13_TestingWithMock() {
    ...
    productionContainer.registerFactory('fileSystem', (path) => new RealFileSystem(path));
    // テスト用コンテナ
    const testContainer = new DIContainer();
    // モックロガー
    const mockLogger = {
        info: (msg) => console.log('[MOCK]', msg),
        error: (msg) => console.error('[MOCK]', msg),
    };
    testContainer.registerInstance('logger', mockLogger);
    // モックファイルシステム
    testContainer.registerFactory('fileSystem', (path) => ({
    ...
    // テストで使用
    const logger = testContainer.resolve('logger');
...
//# sourceMappingURL=di-container.examples.js.map