...
function generateMarkdownReport(report) {
    ...
    for (const result of report.parser) {
        ...
        lines.push(`| ${result.fileSize} | ${result.taskCount} | ${result.parseTimeMs}ms | ${result.threshold}ms | ${status} | ${result.memoryUsedMB}MB |`);
    }
    lines.push('');
    // Serializer Results
    ...
    for (const result of report.serializer) {
        const status = result.passed ? '✅' : '❌';
        lines.push(`| ${result.testName} | ${result.taskCount} | ${result.serializeTimeMs}ms | ${result.threshold}ms | ${status} | ${result.throughputTasksPerSec} tasks/sec |`);
    }
    lines.push('');
    // Diff Results
    ...
    for (const result of report.diff) {
        const status = result.passed ? '✅' : '❌';
        lines.push(`| ${result.testName} | ${result.contentSize} | ${result.diffTimeMs}ms | ${result.threshold}ms | ${status} | ${result.throughputCharsPerSec} chars/sec |`);
    }
    lines.push('');
    // Sync Results
    ...
    for (const result of report.sync) {
        const status = result.passed ? '✅' : '❌';
        lines.push(`| ${result.testName} | ${result.taskCount} | ${result.syncTimeMs}ms | ${result.threshold}ms | ${status} | ${result.throughputTasksPerSec} tasks/sec |`);
    }
    lines.push('');
    // Acceptance Criteria
...
async function main() {
    ...
    const totalTests = parserResults.length +
        serializerResults.length +
        diffResults.length +
        syncResults.length;
    const passedTests = parserCount.passed +
...
//# sourceMappingURL=run_benchmarks.js.map