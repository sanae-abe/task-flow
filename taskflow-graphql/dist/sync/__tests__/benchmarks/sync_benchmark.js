 ...
 */
async function generateMarkdownFromTasks(tasks) {
 ...
 */
async function benchmarkFileToAppSync(taskCount, threshold) {
 ...
 */
async function benchmarkAppToFileSync(taskCount, threshold) {
    const mockDb = new MockDatabase();
    await mockDb.initialize();
    const config = {
        todoPath: '/tmp/benchmark-todo.md',
 ...
 */
async function benchmark3WayMerge(taskCount, conflictPercentage = 10) {
    const mockDb = new MockDatabase();
    await mockDb.initialize();
    const config = {
        todoPath: '/tmp/benchmark-todo.md',
 ...
 */
async function benchmarkBatchWriteOptimization() {
    const mockDb = new MockDatabase();
    await mockDb.initialize();
 ...
 */
export async function runSyncBenchmarks() {
    ...
    for (const test of fileToAppTests) {
        ...
        console.log(`   ${status} ${test.taskCount} tasks: ${result.syncTimeMs}ms / ${result.threshold}ms (${percentage}%)`);
    ...
    for (const test of mergeTests) {
        const result = await benchmark3WayMerge(test.taskCount, test.conflictPercentage);
        results.push(result);
        const status = result.passed ? '✅ PASS' : '❌ FAIL';
        console.log(`   ${status} ${test.taskCount} tasks, ${test.conflictPercentage}% conflicts: ${result.syncTimeMs}ms`);
    ...
    console.log(`        Individual writes: ${batchResult.individualWrites}ms (N+1 problem)`);
    ...
    const avgFileToAppTime = fileToAppTests.reduce((sum, _, idx) => sum + results[idx].syncTimeMs, 0) / fileToAppTests.length;
    const avgAppToFileTime = appToFileTests.reduce((sum, _, idx) => sum + results[fileToAppTests.length + idx].syncTimeMs, 0) / appToFileTests.length;
    console.log('\n📈 Summary:');
    console.log(`   Overall: ${allPassed ? '✅ ALL PASSED' : '❌ SOME FAILED'}`);
    console.log(`   Avg File→App sync time: ${Math.round(avgFileToAppTime * 100) / 100}ms`);
    console.log(`   Avg App→File sync time: ${Math.round(avgAppToFileTime * 100) / 100}ms`);
...
//# sourceMappingURL=sync_benchmark.js.map