...
function generateTodoContent(taskCount) {
    ...
    for (let i = 0; i < Math.min(sectionsPerTasks, taskCount); i++) {
        ...
        lines.push(`- [${checked}] Task ${i + 1} (優先度:高, 期限:2025-12-31) #urgent #important`);
    }
    lines.push('');
    ...
    for (let i = startIdx; i < endIdx; i++) {
        ...
        lines.push(`- [${checked}] Task ${i + 1} (優先度:中, 期限:2025-12-15) #feature`);
    }
    lines.push('');
...
async function benchmarkFileSize(taskCount, threshold) {
    ...
    if (!tasksMatch) {
        console.warn(`⚠️  Task count mismatch: extracted ${extractedTasks.length}, expected ${result.checkboxes.length}`);
    }
    return {
...
export async function stressTestParser(taskCount = 10000) {
    ...
    console.log(`⚡ Throughput: ${Math.round((taskCount / parseTimeMs) * 1000)} tasks/sec`);
    if (parseTimeMs > 5000) {
