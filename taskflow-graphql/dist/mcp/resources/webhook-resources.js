 ...
 */
import { getAllWebhooks, getWebhook, getWebhookDeliveriesByWebhookId, } from '../../utils/indexeddb.js';
/**
 ...
 */
export async function handleWebhookListResource() {
    try {
 ...
 */
export async function handleWebhookDetailResource(id) {
    try {
            ...
            contents: [
                {
                    ...
                    text: JSON.stringify({
                        ...
                        successRate: (webhook.deliveryCount ?? 0) > 0
 ...
 */
export async function handleWebhookDeliveriesResource(id) {
    try {
        const webhook = await getWebhook(id);
        if (!webhook) {
            throw new Error(`Webhook not found: ${id}`);
        ...
        const avgResponseTime = deliveries.length > 0
            ...
            : 0;
        // Group by event type
        const byEvent = deliveries.reduce((acc, d) => {
            if (!acc[d.event]) {
 ...
 */
export async function handleWebhookStatsResource() {
    try {
        const webhooks = await getAllWebhooks();
        const stats = {
            ...
            totalDeliveries: webhooks.reduce((sum, w) => sum + (w.deliveryCount ?? 0), 0),
            totalFailures: webhooks.reduce((sum, w) => sum + (w.failureCount ?? 0), 0),
            ...
            contents: [
                {
                    ...
                    text: JSON.stringify({
                        ...
                        topWebhooks: webhooks
                            ...
                            successRate: (w.deliveryCount ?? 0) > 0
 ...
 */
export async function handleWebhookResource(uri) {
    if (uri === 'webhook://list') {
...
//# sourceMappingURL=webhook-resources.js.map