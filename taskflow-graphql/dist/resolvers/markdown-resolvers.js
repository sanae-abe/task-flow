...
import { getBoard, getTask, getTasksByBoard, getLabelsByBoard, getAllLabels, } from '../utils/indexeddb.js';
import { generateTaskMarkdown, generateTasksMarkdown, generateBoardMarkdown, MarkdownFormat as MdFormat, } from '../utils/markdown-generator.js';
// ============================================================================
// Helper Functions
// ============================================================================
/**
 ...
 */
function convertMarkdownFormat(format) {
    if (!format)
...
function applyTaskFilters(tasks, filters) {
    ...
    if (filters.labels && filters.labels.length > 0) {
        filteredTasks = filteredTasks.filter(task => filters.labels.some(labelId => task.labels.includes(labelId)));
    }
    // Filter by due date range
    if (filters.dueBefore) {
        filteredTasks = filteredTasks.filter(task => task.dueDate && new Date(task.dueDate) <= new Date(filters.dueBefore));
    }
    if (filters.dueAfter) {
        filteredTasks = filteredTasks.filter(task => task.dueDate && new Date(task.dueDate) >= new Date(filters.dueAfter));
    }
    // Filter by search query
    if (filters.search) {
        const searchLower = filters.search.toLowerCase();
        filteredTasks = filteredTasks.filter(task => task.title.toLowerCase().includes(searchLower) ||
...
export const markdownQueries = {
    ...
    exportTaskAsMarkdown: async (_parent, { taskId }) => {
        ...
        const taskLabels = allLabels.filter(label => task.labels.includes(label.id));
        // Generate markdown
        const options = {
            format: MdFormat.STANDARD,
            includeSubtasks: true,
...
// Mutation Resolvers
// ============================================================================
export const markdownMutations = {
    /**
     * Generate comprehensive Markdown report with custom options
     */
    generateMarkdownReport: async (_parent, { input }) => {
        const { boardId, includeCompleted = true, includeSubtasks = true, includeLabels = true, includeAttachments = true, format, filters, } = input;
        // Get board
        const board = await getBoard(boardId);
        if (!board) {
            throw new GraphQLError(`Board not found: ${boardId}`, {
...
//# sourceMappingURL=markdown-resolvers.js.map