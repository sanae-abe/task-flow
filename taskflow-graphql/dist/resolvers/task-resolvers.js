...
import { getTask, getAllTasks, getTasksByBoard, createTask as createTaskDB, updateTask as updateTaskDB, createTasks as createTasksDB, } from '../utils/indexeddb.js';
import { publishEvent, SUBSCRIPTION_TOPICS, subscribe, } from '../utils/pubsub.js';
import { triggerTaskCreated, triggerTaskUpdated, triggerTaskCompleted, triggerTaskDeleted, } from '../utils/webhook-events.js';
import { breakdownTaskWithAI, validateBreakdown, } from '../utils/ai-task-breakdown.js';
import { parseNaturalLanguageToTask, convertToCreateTaskInput, } from '../utils/ai-natural-language.js';
...
import { buildUserContext, validateAIResponse, safeAIOperation, } from '../utils/ai-helpers.js';
// ============================================================================
// Query Resolvers
// ============================================================================
export const taskQueries = {
    ...
    tasks: async (_parent, args) => {
        ...
        if (args.labels && args.labels.length > 0) {
            tasks = tasks.filter(task => args.labels.some(labelId => task.labels.includes(labelId)));
        }
        // Filter by due date range
        if (args.dueBefore) {
            tasks = tasks.filter(task => task.dueDate && new Date(task.dueDate) <= new Date(args.dueBefore));
        }
        if (args.dueAfter) {
            tasks = tasks.filter(task => task.dueDate && new Date(task.dueDate) >= new Date(args.dueAfter));
        }
    ...
    taskStatistics: async (_parent, { boardId }) => {
        ...
        if (completedWithTimes.length > 0) {
            ...
            averageCompletionTime = Math.round(totalTime / completedWithTimes.length / (1000 * 60 * 60)); // Convert to hours
        }
        // Count overdue tasks
        const overdueCount = tasks.filter(t => t.status !== 'COMPLETED' &&
            t.status !== 'DELETED' &&
            t.dueDate &&
    ...
    nextRecommendedTask: async (_parent, { boardId }) => {
        ...
        const userContext = buildUserContext(completedTasks.filter(t => t.status === 'COMPLETED'), new Date());
        // Get AI recommendation
        const recommendation = await safeAIOperation(async () => await getRecommendedTaskWithAI(incompleteTasks, userContext), null);
    ...
    searchTasksByNaturalLanguage: async (_parent, { query }) => {
        ...
        return tasks.filter(task => task.title.toLowerCase().includes(queryLower) ||
...
export const taskMutations = {
    ...
    createTask: async (_parent, { input }, _context) => {
        ...
        await triggerTaskCreated(newTask);
        return newTask;
    },
    /**
    ...
    updateTask: async (_parent, { id, input }, _context) => {
        ...
        if (input.subtasks !== undefined && input.subtasks !== null) {
            updates.subtasks = input.subtasks.map((st) => {
                if (st.id) {
                    return {
                        id: st.id,
                        title: st.title ?? '',
        ...
        if (input.files !== undefined && input.files !== null) {
            updates.files = input.files.map((file) => ({
                id: crypto.randomUUID(),
                name: file.name,
                type: file.type,
                size: file.size,
    ...
    deleteTask: async (_parent, { id }, _context) => {
        ...
        if (deleted) {
            ...
            await triggerTaskDeleted(deleted);
        }
        return true;
    },
    /**
     ...
     */
    moveTask: async (_parent, { id, targetColumnId, targetPosition }, _context) => {
                    ...
                    preferences: aiContext.preferences
                        ? {
                            workingHours: aiContext.preferences.workingHours
                                ...
                                : undefined,
                            preferredPriority: aiContext.preferences.preferredPriority || undefined,
                            autoBreakdownEnabled: aiContext.preferences.autoBreakdownEnabled || undefined,
                        }
                        : undefined,
                }
                : undefined,
    ...
    breakdownTask: async (_parent, { taskId, strategy }) => {
        ...
        const subtasksData = breakdownResult.subtasks.map((subtaskTitle, index) => ({
            boardId: original.boardId,
            columnId: original.columnId,
            title: subtaskTitle,
    ...
    optimizeTaskSchedule: async (_parent, { boardId, constraints }) => {
        ...
        const optimizationConstraints = {
            ...
            prioritizeBy: (constraints?.prioritizeBy || 'CRITICAL'),
        };
...
export const taskFieldResolvers = {
     ...
     */
    isOverdue: parent => {
        if (!parent.dueDate || parent.status === 'COMPLETED')
            return false;
    ...
    labels: async (parent, _args, context) => {
        const labels = await Promise.all(parent.labels.map(labelId => context.loaders.labelLoader.load(labelId)));
...
export const taskSubscriptions = {
    taskCreated: {
        subscribe: (_parent, { boardId: _boardId }) => subscribe(SUBSCRIPTION_TOPICS.TASK_CREATED),
    },
    taskUpdated: {
        subscribe: (_parent, { boardId: _boardId }) => subscribe(SUBSCRIPTION_TOPICS.TASK_UPDATED),
    },
    taskCompleted: {
        subscribe: (_parent, { boardId: _boardId }) => subscribe(SUBSCRIPTION_TOPICS.TASK_COMPLETED),
    },
    taskDeleted: {
        subscribe: (_parent, { boardId: _boardId }) => subscribe(SUBSCRIPTION_TOPICS.TASK_DELETED),
    },
};
//# sourceMappingURL=task-resolvers.js.map