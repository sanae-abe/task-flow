...
(function (MarkdownFormat) {
    ...
    MarkdownFormat["GITHUB_FLAVORED"] = "GITHUB_FLAVORED";
 ...
 */
export function generateTaskMarkdown(task, labels = [], options = {}) {
    const { format = MarkdownFormat.STANDARD, includeSubtasks = true, includeLabels = true, includeAttachments = true, } = options;
    let markdown = '';
    ...
    // Due date
    if (task.dueDate) {
        const dueInfo = formatDueDate(task.dueDate, task.dueTime, task.status === 'COMPLETED');
        markdown += `  ${dueInfo}\n`;
    }
    // Subtasks
 ...
 */
export function generateTasksMarkdown(tasks, columns, labels = [], options = {}) {
    let markdown = '';
 ...
 */
export function generateBoardMarkdown(board, tasks, labels = [], options = {}) {
 ...
 */
function formatPriority(priority, format) {
    const priorityMap = {
        CRITICAL: '🔴 Critical',
        HIGH: '🟠 High',
        MEDIUM: '🟡 Medium',
 ...
 */
function formatLabels(labels, format) {
    if (labels.length === 0)
        return '';
    switch (format) {
        case MarkdownFormat.OBSIDIAN:
 ...
 */
function formatDueDate(dueDate, dueTime, isCompleted = false) {
 ...
 */
function formatAttachments(files) {
 ...
 */
function generateObsidianFrontmatter(board, tasks) {
 ...
 */
function generateMetadataFooter(board, tasks) {
    const now = new Date();
    const stats = calculateStats(tasks);
    let markdown = '\n---\n\n';
 ...
 */
export function generateMarkdownFilename(boardName, _mdFormat = MarkdownFormat.STANDARD) {
 ...
 */
export function getMarkdownExtension(_mdFormat) {
