...
import type { SyncConfig, FileWatcherEvent } from '../types';
...
async function unifiedEventExample() {
  ...
  // すべてのイベントを統一的に処理
  watcher.on('event', (event: FileWatcherEvent) => {
    switch (event.type) {
      case 'change':
...
// ヘルパー関数
function handleFileChange(event: FileWatcherEvent) {
  console.log('Handling file change:', event.path);
  // 同期処理をトリガー
}

function handleFileAdd(event: FileWatcherEvent) {
  console.log('Handling file add:', event.path);
  // 初期同期をトリガー
}

function handleFileDelete(event: FileWatcherEvent) {
  console.log('Handling file delete:', event.path);
  // 削除処理をトリガー
}

function handleError(event: FileWatcherEvent) {
