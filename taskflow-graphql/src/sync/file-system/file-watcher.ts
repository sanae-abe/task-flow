...
export class FileWatcher extends EventEmitter {
  ...
  private validatedPath: string;
  private retryTimeouts: Map<string, NodeJS.Timeout> = new Map();

  // レート制限された各イベントハンドラ
  private rateLimitedHandlers: Map<
    string,
  ...
  constructor(options: FileWatcherOptions) {
    ...
    try {
      ...
      this.logger.debug(
        { validatedPath: this.validatedPath },
        'File path validated successfully'
      );
    } catch (error) {
      this.logger.error(
        { err: error, filePath: this.options.filePath },
   ...
   */
  private setupRateLimitedHandlers(): void {
    const eventTypes: Array<'change' | 'add' | 'unlink'> = [
      'change',
      'add',
      'unlink',
    ];
    ...
    for (const eventType of eventTypes) {
      const handler = this.rateLimiter.rateLimit(
    ...
    try {
      ...
      if (!exists) {
        this.logger.warn(
          ...
          'File does not exist yet, waiting for creation'
        );
      }
  ...
  private handleError(error: Error): void {
    const errorEvent: FileWatcherEvent = {
      type: 'error',
      path: this.validatedPath,
      timestamp: new Date(),
      error,
   ...
   * ファイルサイズを検証
   */
  private async validateFileSize(): Promise<void> {
    try {
   ...
   * 統計情報を更新
   */
  private updateStatistics(event: FileWatcherEvent): void {
  ...
  getRateLimiterStats(): Map<string, any> {
    return this.rateLimiter.getStats() as Map<string, any>;
  }
  ...
  async getFileInfo(): Promise<{
    ...
    try {
      ...
      const stats = await fs.stat(this.validatedPath);
      const isReadable = await this.pathValidator.isReadable(
        this.validatedPath
      );
      const isWritable = await this.pathValidator.isWritable(
        this.validatedPath
      );
