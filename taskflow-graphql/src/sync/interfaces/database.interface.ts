...
export interface Database {
  ...
  // --------------------------------------------------------------------------
  // CRUD Operations - Webhooks (Optional - Supabase専用)
  // --------------------------------------------------------------------------
  ...
  getWebhookDeliveries?(
    webhookId: string,
    options?: QueryOptions
  ): Promise<QueryResult<WebhookDeliveryRecord>>;
 ...
 */
export function isDatabaseImplementation(obj: unknown): obj is Database {
  if (typeof obj !== 'object' || obj === null) return false;

  const db = obj as Database;
  return (
    typeof db.getTask === 'function' &&
 ...
 * バッチ操作結果が完全成功かチェック
 */
export function isBatchOperationSuccess<T>(
 ...
 * クエリ結果が空かチェック
 */
export function isQueryResultEmpty<T>(result: QueryResult<T>): boolean {
