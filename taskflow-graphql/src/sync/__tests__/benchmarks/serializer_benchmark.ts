 ...
 */
async function benchmarkFullSerialization(
  taskCount: number,
  threshold: number
): Promise<SerializerBenchmarkResult> {
 ...
 */
async function benchmarkDifferentialUpdate(): Promise<SerializerBenchmarkResult> {
  const serializer = new MarkdownSerializer();
  ...
  // Update single task
  const taskToUpdate = { ...tasks[50], status: 'completed' as const };
 ...
 */
async function benchmarkTaskAddition(): Promise<SerializerBenchmarkResult> {
  const serializer = new MarkdownSerializer();
 ...
 */
export async function runSerializerBenchmarks(): Promise<SerializerBenchmarkResult[]> {
  console.log('\n📊 Serializer Performance Benchmark\n');
  console.log('='.repeat(80));

  const results: SerializerBenchmarkResult[] = [];
  ...
  for (const test of fullSerializationTests) {
    ...
    const percentage = Math.round(
      (result.serializeTimeMs / result.threshold) * 100
    );

    console.log(
      `   ${status} ${test.taskCount} tasks: ${result.serializeTimeMs}ms / ${result.threshold}ms (${percentage}%)`
    );
    console.log(
      `        Output: ${result.outputSizeLines} lines, ${result.outputSizeChars} chars`
    );
    console.log(
  ...
  console.log(
    `   ${diffStatus} ${diffResult.testName}: ${diffResult.serializeTimeMs}ms / ${diffResult.threshold}ms`
  );
  ...
  console.log(
    `   ${addStatus} ${addResult.testName}: ${addResult.serializeTimeMs}ms / ${addResult.threshold}ms`
  );
  ...
  const avgTime =
    fullSerializationResults.reduce((sum, r) => sum + r.serializeTimeMs, 0) /
    fullSerializationResults.length;

  console.log('\n📈 Summary:');
  console.log(`   Overall: ${allPassed ? '✅ ALL PASSED' : '❌ SOME FAILED'}`);
  console.log(
    `   Avg full serialization time: ${Math.round(avgTime * 100) / 100}ms`
  );
  console.log(
    `   Differential update performance: ${diffResult.serializeTimeMs}ms per update`
  );
  console.log(
    `   Task addition performance: ${addResult.serializeTimeMs}ms per add`
  );
