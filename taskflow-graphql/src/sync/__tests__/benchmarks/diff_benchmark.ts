...
function generateMarkdownContent(lineCount: number): string {
  ...
  for (let i = 0; i < lineCount - 5; i++) {
    if (i % 15 === 0) {
      ...
      lines.push(
        `- [${checked}] Task ${taskId + 1}: This is a sample task #tag${taskId % 5}`
      );
      taskId++;
    }
  }
 ...
 */
function modifyContent(content: string, changePercentage: number = 10): string {
 ...
 */
async function benchmarkCharacterLevelDiff(
  lineCount: number,
  threshold: number
): Promise<DiffBenchmarkResult> {
 ...
 */
async function benchmarkLineLevelDiff(
  lineCount: number,
  threshold: number
): Promise<DiffBenchmarkResult> {
  const detector = new DiffDetector();
 ...
 */
async function benchmarkIdenticalDetection(): Promise<DiffBenchmarkResult> {
  const detector = new DiffDetector();
 ...
 */
async function benchmarkChangeDetectionAccuracy(): Promise<{
  testName: string;
  accuracy: number;
 ...
 */
export async function runDiffBenchmarks(): Promise<DiffBenchmarkResult[]> {
  console.log('\n📊 DiffDetector Performance Benchmark\n');
  console.log('='.repeat(80));

  const results: DiffBenchmarkResult[] = [];
  ...
  for (const test of charLevelTests) {
    ...
    console.log(
      `   ${status} ${test.lineCount} lines: ${result.diffTimeMs}ms / ${result.threshold}ms (${percentage}%)`
    );
    console.log(
      `        Changes: ${result.changesDetected} chars, Throughput: ${result.throughputCharsPerSec} chars/sec`
    );
  }
  ...
  for (const test of lineLevelTests) {
    ...
    console.log(
      `        Changes: ${result.changesDetected} lines, Throughput: ${result.throughputCharsPerSec} chars/sec`
    );
  }
  ...
  console.log(
    `   ${identicalStatus} ${identicalResult.testName}: ${identicalResult.diffTimeMs}ms / ${identicalResult.threshold}ms`
  );
  console.log(
    `        Throughput: ${identicalResult.throughputCharsPerSec} chars/sec`
  );
  ...
  console.log(
    `   ${accuracyStatus} ${accuracyResult.testName}: ${accuracyResult.accuracy}%`
  );
  console.log(
    `        False positives: ${accuracyResult.falsePositives}, False negatives: ${accuracyResult.falseNegatives}`
  );
  ...
  const avgCharLevelTime =
    charLevelTests.reduce(
      (sum, _, idx) => sum + results[idx].diffTimeMs,
      0
    ) / charLevelTests.length;
  const avgLineLevelTime =
    lineLevelTests.reduce(
      (sum, _, idx) => sum + results[charLevelTests.length + idx].diffTimeMs,
      0
    ) / lineLevelTests.length;

  console.log('\n📈 Summary:');
  console.log(`   Overall: ${allPassed ? '✅ ALL PASSED' : '❌ SOME FAILED'}`);
  console.log(
    `   Avg char-level diff time: ${Math.round(avgCharLevelTime * 100) / 100}ms`
  );
  console.log(
    `   Avg line-level diff time: ${Math.round(avgLineLevelTime * 100) / 100}ms`
  );
