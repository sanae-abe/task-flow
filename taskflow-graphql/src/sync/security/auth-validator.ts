...
export class AuthValidator {
  private readonly MCP_AUTH_TOKEN_SCHEMA = z.string().min(32);
  ...
  constructor() {
    ...
    if (process.env.NODE_ENV === 'production' && !this.expectedToken) {
      throw new Error(
        'SECURITY ERROR: MCP_AUTH_TOKEN must be set in production environment. ' +
    ...
    if (!this.expectedToken && process.env.NODE_ENV !== 'test') {
      console.warn(
        '⚠️  MCP_AUTH_TOKEN is not set. MCP Tool access will be denied. ' +
