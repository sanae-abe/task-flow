...
export class BatchWriter {
   ...
   * @returns 書き込まれたタスク数
   */
  async bulkUpsertTasks(tasks: Task[]): Promise<number> {
    if (!this.db) {
   ...
   * @returns タスクの配列
   */
  async bulkGetTasks(taskIds: string[]): Promise<(Task | undefined)[]> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
  ...
  async transaction(
    ...
    const promises = operations.map(op => {
      switch (op.type) {
        case 'put':
          return store.put(op.data as Task);
        case 'delete':
          return store.delete(op.data as string);
        case 'get':
          return store.get(op.data as string);
      }
    });
