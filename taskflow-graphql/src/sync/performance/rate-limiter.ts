...
export class RateLimiter {
  ...
  debounce<T extends (...args: any[]) => any>(
    ...
    // 実行カウントを追跡
    const originalFunc = debouncedFunc as any;
    const trackedFunc = ((...args: any[]) => {
  ...
  throttle<T extends (...args: any[]) => any>(
    ...
    // 実行カウントを追跡
    const originalFunc = throttledFunc as any;
    const trackedFunc = ((...args: any[]) => {
      const result = originalFunc(...args);
      this.incrementExecutionCount(funcName);
      return result;
   ...
   * @param funcName 関数名
   */
  private incrementCallCount(funcName: string): void {
   ...
   * @param funcName 関数名
   */
  private incrementExecutionCount(funcName: string): void {
    const stats = this.stats.get(funcName);
    if (stats) {
      stats.executedCalls++;
   ...
   * @param funcName 関数名
   */
  private updateSkipRate(funcName: string): void {
    const stats = this.stats.get(funcName);
    if (stats) {
  ...
  formatStats(funcName?: string): string {
    const stats = funcName
      ? new Map([[funcName, this.getStats(funcName) as ExecutionStats]])
      : (this.getStats() as Map<string, ExecutionStats>);
  ...
  getPerformanceReport(): {
    ...
    const allStats = this.getStats() as Map<string, ExecutionStats>;

    let totalCalls = 0;
    let totalExecutions = 0;
    let totalSkipped = 0;

    const functions: Array<{ name: string; stats: ExecutionStats }> = [];
    ...
    const averageSkipRate =
      totalCalls > 0 ? Math.round((totalSkipped / totalCalls) * 100) : 0;
