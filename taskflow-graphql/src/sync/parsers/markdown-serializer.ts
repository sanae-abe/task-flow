...
export class MarkdownSerializer {
  private readonly DEFAULT_OPTIONS: MarkdownSerializeOptions = {
    includeFrontMatter: true,
    indent: '  ',
    sectionSpacing: 1,
    checkboxFormat: {
  ...
  async serialize(
    ...
    try {
      ...
      logger.debug(
        ...
        'Starting markdown serialization'
      );
      ...
      for (const [sectionName, sectionTasks] of tasksBySection) {
        ...
        const sectionMarkdown = this.serializeSection(
          sectionName,
          sectionTasks,
          opts,
          context
      ...
      if (context.errors.length > 0) {
        logger.warn(
          ...
          'Serialization completed with errors'
        );
      }
   ...
   */
  private generateFrontMatter(tasks: Task[]): string {
    const lines: string[] = [];
    lines.push('---');
   ...
   */
  private groupTasksBySection(tasks: Task[]): Map<string, Task[]> {
  ...
  updateTaskLine(content: string, task: Task, lineNumber: number): string {
    ...
    if (lineNumber < 0 || lineNumber >= lines.length) {
      logger.warn(
        { lineNumber, totalLines: lines.length },
        'Invalid line number'
      );
      return content;
    }
