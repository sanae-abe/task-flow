...
export class MockDatabase implements Database {
  ...
  async getTasks(ids: string[]): Promise<TaskRecord[]> {
    this.ensureConnected();
    return ids
  ...
  ): Promise<QueryResult<TaskRecord>> {
    this.ensureConnected();
    const allTasks = Array.from(this.store.tasks.values()).filter(
      (task) => task.boardId === boardId && task.status !== 'DELETED'
    );
  ...
  async getTasksByColumn(
    ...
    this.ensureConnected();
    const allTasks = Array.from(this.store.tasks.values()).filter(
      (task) => task.columnId === columnId && task.status !== 'DELETED'
    );
    return this.applyQueryOptions(allTasks, options);
  }
  ...
  async deleteTasks(ids: string[]): Promise<BatchOperationResult<string>> {
    this.ensureConnected();
    const success: string[] = [];
    const failed: Array<{ record: string; error: Error }> = [];
  ...
  async getBoards(
    options?: QueryOptions
  ): Promise<QueryResult<BoardRecord>> {
    this.ensureConnected();
  ...
  async getLabel(id: string): Promise<LabelRecord | null> {
    this.ensureConnected();
    return this.store.labels.get(id) ?? null;
  }

  async getLabelsByBoard(boardId?: string): Promise<LabelRecord[]> {
    this.ensureConnected();
    ...
    return allLabels.filter(
      (label) => label.boardId === boardId || !label.boardId
    );
  }
  ...
  async getTemplates(
    options?: QueryOptions
  ): Promise<QueryResult<TemplateRecord>> {
    this.ensureConnected();
  ...
  async batchCreateTasks(
    ...
    this.ensureConnected();
    const success: TaskRecord[] = [];
    const failed: Array<{ record: TaskRecord; error: Error }> = [];
  ...
  async batchUpdateTasks(
    updates: Array<{ id: string; updates: Partial<TaskRecord> }>
  ): Promise<BatchOperationResult<TaskRecord>> {
    this.ensureConnected();
    const success: TaskRecord[] = [];
    const failed: Array<{ record: TaskRecord; error: Error }> = [];
  ...
  async batchCreateLabels(
    labels: LabelRecord[]
  ): Promise<BatchOperationResult<LabelRecord>> {
    this.ensureConnected();
    const success: LabelRecord[] = [];
    const failed: Array<{ record: LabelRecord; error: Error }> = [];
  ...
  async getTaskStats(boardId?: string): Promise<DatabaseStats> {
    this.ensureConnected();
    const tasks = boardId
      ? Array.from(this.store.tasks.values()).filter(
  ...
  async getBoardStats(): Promise<DatabaseStats> {
    this.ensureConnected();
    const boards = Array.from(this.store.boards.values());
    const lastModified =
      boards.length > 0
        ? new Date(
            Math.max(...boards.map((b) => new Date(b.updatedAt).getTime()))
          )
        : new Date();
  ...
  async getLabelStats(boardId?: string): Promise<DatabaseStats> {
    this.ensureConnected();
    const labels = await this.getLabelsByBoard(boardId);
    const lastModified =
      labels.length > 0
        ? new Date(
            Math.max(...labels.map((l) => new Date(l.updatedAt).getTime()))
          )
        : new Date();
  ...
  async getTasksByLabel(
    ...
    this.ensureConnected();
    const tasks = Array.from(this.store.tasks.values()).filter(
      (task) => task.labels.includes(labelId) && task.status !== 'DELETED'
    );
    return this.applyQueryOptions(tasks, options);
  }
  ...
  async purgeDeletedTasks(retentionDays: number): Promise<string[]> {
    this.ensureConnected();
    const now = new Date();
  ...
  private applyQueryOptions<T>(
    ...
    const hasMore =
      options?.limit !== undefined && result.length === options.limit
  ...
  private estimateStorageSize(items: unknown[]): number {
    // 簡易的な推定（JSON文字列長）
