...
import {
  getWebhook,
  getAllWebhooks,
  createWebhook as createWebhookDB,
  updateWebhook as updateWebhookDB,
  deleteWebhook as deleteWebhookDB,
  getWebhookDelivery as getWebhookDeliveryDB,
...
function toWebhookType(record: WebhookRecord): Webhook {
  return {
    ...record,
...
function toWebhookDeliveryType(record: WebhookDeliveryRecord): WebhookDelivery {
  return {
    ...record,
...
export const webhookQueries: Pick<
  QueryResolvers,
  | 'webhook'
  | 'webhooks'
  | 'webhookDelivery'
...
export const webhookMutations: Pick<
  MutationResolvers,
...
> = {
  ...
  createWebhook: async (_parent, { input }, _context) => {
    ...
    const webhookData: Omit<WebhookRecord, 'id' | 'createdAt' | 'updatedAt'> = {
      url: input.url,
      events: input.events as any[], // GraphQL codegen will handle proper typing
      active: true,
  ...
  updateWebhook: async (_parent, { id, input }, _context) => {
    ...
    if (input.events !== undefined && input.events !== null) {
      updates.events = input.events as any[];
    }
