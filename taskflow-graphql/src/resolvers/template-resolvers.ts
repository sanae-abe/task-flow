...
export const templateQueries: Pick<QueryResolvers, 'template' | 'templates'> = {
  /**
   * Get single template by ID
   */
  template: async (_parent, { id }, context) =>
    await context.loaders.templateLoader.load(id),
...
export const templateMutations: Pick<
  MutationResolvers,
...
> = {
  ...
  createTemplate: async (_parent, { input }, _context) => {
    const taskTemplate: TaskTemplateDataRecord = {
      ...
      recurrence: input.taskTemplate.recurrence
        ? (input.taskTemplate.recurrence as RecurrenceConfigRecord)
        : undefined,
      subtasks: (input.taskTemplate.subtasks ?? []).map(
    ...
    if (input.taskTemplate !== undefined && input.taskTemplate !== null) {
      ...
      updates.taskTemplate = {
        ...
        recurrence: tt.recurrence
          ? (tt.recurrence as RecurrenceConfigRecord)
          : undefined,
        subtasks: (tt.subtasks ?? []).map(
          (st: any, index: number): SubTaskRecord => ({
...
export const templateFieldResolvers: TemplateResolvers = {
  /**
