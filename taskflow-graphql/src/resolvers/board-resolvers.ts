...
export const boardQueries: Pick<
  QueryResolvers,
...
> = {
  ...
  board: async (_parent, { id }, context) =>
    await context.loaders.boardLoader.load(id),
...
export const boardMutations: Pick<
  MutationResolvers,
...
> = {
  ...
  createBoard: async (_parent, { input }, _context) => {
    ...
    const settings: BoardSettingsRecord = {
    ...
    await triggerBoardCreated(newBoard as any);
  ...
  updateBoard: async (_parent, { id, input }, _context) => {
    ...
    if (input.name !== undefined) updates.name = input.name ?? undefined;
    if (input.description !== undefined)
      updates.description = input.description ?? undefined;

    if (input.columns !== undefined && input.columns !== null) {
      updates.columns = input.columns.map(
        (col: any, index: number): BoardColumnRecord => ({
          id: col.id || crypto.randomUUID(),
          name: col.title, // Use 'name' field for BoardColumnRecord
          position: col.position ?? index,
          taskIds: [], // Initialize empty task list
    ...
    if (input.settings !== undefined && input.settings !== null) {
      ...
      updates.settings = {
        ...existing.settings,
        defaultColumn:
          input.settings.defaultColumnId ?? existing.settings.defaultColumn,
        ...
        recycleBinRetentionDays:
          input.settings.recycleBinRetentionDays ?? undefined,
      };
    }
    ...
    // Trigger webhook event
    await triggerBoardUpdated(updated as any);
  ...
  deleteBoard: async (_parent, { id }, _context) => {
    ...
    if (tasks.length > 0) {
      throw new GraphQLError(
        'Cannot delete board with tasks. Please delete all tasks first.',
        {
    ...
    // Trigger webhook event
    await triggerBoardDeleted(board as any);
...
export const boardFieldResolvers: BoardResolvers = {
  /**
...
export const boardSubscriptions: Pick<SubscriptionResolvers, 'boardUpdated'> = {
  boardUpdated: {
    subscribe: (_parent, { boardId: _boardId }) =>
      subscribe(SUBSCRIPTION_TOPICS.BOARD_UPDATED),
  },
};
