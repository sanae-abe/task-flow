...
import type { TemplateRecord } from '../../types/database.js';

/**
 * Template Resource Schemas
 */
export const templateResourceSchemas: Resource[] = [
  {
    uri: 'template://list',
    name: 'All Templates',
 ...
 */
export async function handleTemplateListResource(): Promise<{
          ...
          text: JSON.stringify(
            {
              ...
              templates: templates.map((t: TemplateRecord) => ({
                id: t.id,
                name: t.name,
 ...
 */
export async function handleTemplateDetailResource(id: string): Promise<{
  contents: Array<{ uri: string; mimeType: string; text: string }>;
}> {
  try {
 ...
 */
export async function handleTemplateCategoriesResource(): Promise<{
  contents: Array<{ uri: string; mimeType: string; text: string }>;
}> {
  try {
    const templates = await getAllTemplates();
    const categories = new Map<
      string,
      { name: string; count: number; favorites: number }
    >();

    templates.forEach((template: TemplateRecord) => {
      ...
      cat.count++;
      if (template.isFavorite) cat.favorites++;
    });
 ...
 */
export async function handleTemplateResource(uri: string): Promise<{
  contents: Array<{ uri: string; mimeType: string; text: string }>;
}> {
