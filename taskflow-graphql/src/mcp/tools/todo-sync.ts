...
function validateAuthToken(): boolean {
  ...
  if (token.length < 16) {
    logger.logSecurityEvent(
      'auth_failure',
      { feature: 'mcp-todo-sync' },
      'MCP_AUTH_TOKEN is too short (minimum 16 characters required)'
    );
    return false;
  }
 ...
 */
async function createSyncCoordinator(
  database: IDBPDatabase | undefined,
  options?: TodoSyncArgs['options']
 ...
 */
function formatStatistics(stats: SyncStatistics): string {
  const successRate =
    stats.totalSyncs > 0
 ...
 */
function formatHistory(history: SyncHistory[]): string {
  return JSON.stringify(
    history.map(h => ({
      id: h.id,
...
export const todoSyncTools = [
  {
    name: 'todo_sync',
    description:
      'Synchronize TODO.md file with TaskFlow application. Supports bidirectional sync, status queries, backup, and restore operations.',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
        ...
        options: {
          ...
          properties: {
            ...
            historyLimit: {
              type: 'number',
              description:
                'Number of history entries to return (for status action)',
              minimum: 1,
              maximum: 100,
            },
          },
 ...
 */
async function handleFileToApp(
  ...
  try {
    ...
    logger.error(
      { err: error, action: 'file_to_app' },
      'File to app sync failed'
    );
 ...
 */
async function handleAppToFile(
  coordinator: SyncCoordinator,
  options?: TodoSyncArgs['options']
): Promise<MCPToolResult> {
  ...
  try {
    ...
    logger.error(
      { err: error, action: 'app_to_file' },
      'App to file sync failed'
    );
          ...
          text: JSON.stringify(
            {
              ...
              unresolvedConflicts: {
                ...
                conflicts:
                  conflicts.length > 0 ? formatConflicts(conflicts) : [],
              },
            },
            null,
            2
...
async function handleRestore(
  ...
  try {
    ...
    const { PathValidator } = await import(
      '../../sync/security/path-validator.js'
    );

    // 1. Validate backup path with PathValidator
    const backupValidator = new PathValidator(process.cwd());
    let validatedBackupPath: string;
    ...
    try {
      ...
      logger.logSecurityEvent(
        ...
        `Invalid backup path: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    ...
    const backupReadable =
      await backupValidator.isReadable(validatedBackupPath);
    if (!backupReadable) {
      return {
        content: [
          {
    ...
    try {
      ...
      logger.error(
        { err: error, backupPath: validatedBackupPath },
    ...
    try {
      ...
      logger.logSecurityEvent(
        'path_traversal',
        { todoPath, feature: 'mcp-restore' },
        `Invalid TODO.md path: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    ...
    if (await todoValidator.exists(validatedTodoPath)) {
      ...
      currentBackupPath = path.join(
        dirname,
        `.backup/${basename}_before_restore_${timestamp}.md`
      );
      ...
      try {
        ...
        logger.info(
          { currentBackupPath },
          'Created backup of current TODO.md before restore'
        );
      } catch (error) {
        logger.warn(
          { err: error },
          'Failed to backup current TODO.md before restore'
        );
    ...
    try {
      ...
      logger.info(
        { backupPath: validatedBackupPath, todoPath: validatedTodoPath },
        'Successfully restored backup to TODO.md'
      );
    } catch (error) {
      logger.error(
        { err: error, todoPath: validatedTodoPath },
    ...
    try {
      ...
      logger.info(
        { tasksChanged: syncResult?.tasksChanged || 0 },
        'Successfully synced restored TODO.md to app'
      );
    } catch (error) {
            ...
            text: JSON.stringify(
              {
                ...
                recommendation:
                  'Run file_to_app action manually to complete the restore',
              },
              null,
              2
            ),
...
export async function handleTodoSync(
  ...
  logger.info(
    { action: args.action, options: args.options },
    'TODO sync MCP tool invoked'
  );
  ...
  try {
    ...
    logger.error(
      { err: error, action: args.action },
      'TODO sync MCP tool failed'
    );
