...
import type { TaskRecord } from '../../types/database.js';
import type { OptimizationOptions } from '../../utils/ai-schedule-optimizer.js';
...
export const aiTools: Tool[] = [
  {
    name: 'breakdown_task',
    description:
      'Break down a complex task into actionable subtasks using AI. Supports multiple breakdown strategies including sequential, parallel, and hybrid approaches.',
    inputSchema: {
      type: 'object',
      properties: {
        taskId: {
  ...
  },
  {
    ...
    description:
      'Create a task from natural language description. AI will extract task properties like title, description, priority, and due date.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
          type: 'string',
          description:
            'Natural language task description (e.g., "Create a high priority task to implement user authentication by next Friday")',
        },
        boardId: {
          type: 'string',
        ...
        columnId: {
          type: 'string',
          description:
            'Optional column ID. If not provided, uses default column.',
        },
      },
  ...
  },
  {
    ...
    description:
      'Optimize task schedule using AI to maximize efficiency and meet deadlines. Considers task dependencies, priorities, and resource constraints.',
    inputSchema: {
      type: 'object',
      properties: {
        boardId: {
  ...
  },
  {
    ...
    description:
      'Get AI-recommended next task to work on. Uses intelligent prioritization based on deadlines, dependencies, and current progress.',
    inputSchema: {
      type: 'object',
      properties: {
        boardId: {
        ...
        context: {
          type: 'string',
          description:
            'Optional context (e.g., "frontend development", "urgent fixes")',
        },
      },
      required: ['boardId'],
    },
...
export async function handleCreateTaskFromNaturalLanguage(args: {
  ...
  try {
    ...
    const taskRecord: Omit<TaskRecord, 'id' | 'createdAt' | 'updatedAt'> = {
      ...
      subtasks:
        parsedTask.subtasks?.map((title, index) => ({
          id: `subtask-${index}`,
          title,
          completed: false,
          position: index,
          ...
          text: JSON.stringify(
            {
              ...
              confidence: (parsedTask as unknown as { confidence?: number })
                .confidence,
              extractedEntities: (
                parsedTask as unknown as {
          ...
          text: JSON.stringify(
            {
              ...
              optimization: {
                ...
                estimatedCompletionDate:
                  optimizedSchedule.estimatedCompletionDate,
              },
...
export async function handleAITool(
  ...
  switch (toolName) {
    case 'breakdown_task':
      return handleBreakdownTask(
        args as { taskId: string; strategy?: BreakdownStrategy }
      );
    case 'create_task_from_natural_language':
      return handleCreateTaskFromNaturalLanguage(
        args as { query: string; boardId: string; columnId?: string }
      );
    case 'optimize_schedule':
      return handleOptimizeSchedule(
        args as { boardId: string; workHoursPerDay?: number; teamSize?: number }
      );
    case 'get_recommended_task':
      return handleGetRecommendedTask(
        args as { boardId: string; context?: string }
      );
    default:
