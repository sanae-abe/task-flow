...
export const webhookTools: Tool[] = [
  {
    name: 'create_webhook',
    description:
      'Create a webhook to receive real-time notifications for task and board events. Supports multiple event types and secure delivery.',
    inputSchema: {
      type: 'object',
      properties: {
        url: {
        ...
        secret: {
          type: 'string',
          description:
            'Optional secret for HMAC signature verification (recommended for security)',
          minLength: 16,
        },
        description: {
          type: 'string',
  ...
  },
  {
    name: 'test_webhook',
    description:
      'Test a webhook by sending a test event. Verifies endpoint connectivity and response.',
    inputSchema: {
      type: 'object',
      properties: {
        id: {
...
export async function handleCreateWebhook(args: {
  ...
  try {
    ...
    const webhookData: Omit<WebhookRecord, 'id' | 'createdAt' | 'updatedAt'> = {
      url: args.url,
      events: args.events,
      secret: args.secret,
...
export async function handleUpdateWebhook(args: {
  ...
  try {
    ...
    const updates: Partial<WebhookRecord> = {};
...
export async function handleWebhookTool(
  ...
  switch (toolName) {
    ...
    case 'list_webhooks':
      return handleListWebhooks(args as { active?: boolean });
    case 'test_webhook':
      return handleTestWebhook(args as { id: string });
    case 'update_webhook':
    ...
    case 'delete_webhook':
      return handleDeleteWebhook(args as { id: string });
    case 'get_webhook_deliveries':
      return handleGetWebhookDeliveries(args as { id: string; limit?: number });
    default:
