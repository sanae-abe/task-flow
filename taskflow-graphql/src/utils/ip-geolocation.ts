...
export async function getGeolocation(
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      console.error(
        `Geolocation fetch failed (attempt ${attempt + 1}/${retries + 1}):`,
        error
      );
      if (attempt === retries) {
 ...
 */
function getFromCache(ip: string): GeolocationData | null {
 ...
 * Set cache entry
 */
function setCache(ip: string, data: GeolocationData): void {
 ...
 */
function getFallbackData(ip: string): GeolocationData {
  return {
    ip,
    country: 'Unknown',
    countryCode: 'XX',
