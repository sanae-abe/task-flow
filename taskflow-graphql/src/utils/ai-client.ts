...
class FallbackAIClient implements AIClient {
  ...
  async parseNaturalLanguage(
    ...
    if (
      ...
      queryLower.includes('low priority') ||
    ...
    if (foundActions.length > 1) {
      result.subtasks = foundActions.map(
        action =>
          `${action.charAt(0).toUpperCase() + action.slice(1)} related work`
      );
    }
    ...
    result.title = query
      .replace(
        /\b(today|tomorrow|next week|end of month|urgent|critical|asap|important|high priority|low priority|whenever)\b/gi,
        ''
      )
      .trim();
  ...
  async optimizeSchedule(
    ...
    const overdueTasks = optimizedTasks.filter(
    ...
    if (overdueTasks.length > 0) {
      suggestions.push(
        `Address ${overdueTasks.length} overdue task(s) immediately`
      );
    }
    if (
      ...
      suggestions.push(
        'Consider increasing team size or working hours to meet deadline'
      );
    }
  ...
  async getRecommendedTask(
    ...
    const scoredTasks = incompleteTasks.map(task => {
      ...
      if (
        userContext.completionHistory &&
        userContext.completionHistory.length > 0
      ) {
        const similarCompletions = userContext.completionHistory.filter(
          h => Math.abs(h.timeOfDay - currentHour) <= 2 // Within 2 hours
        );
   ...
   */
  private estimateComplexity(task: TaskRecord): number {
    let complexity = 5; // Base complexity

    // More subtasks = higher complexity
    if (task.subtasks.length > 5) complexity += 2;
    else if (task.subtasks.length > 2) complexity += 1;
...
Example: ["Subtask 1", "Subtask 2", "Subtask 3"]`;
      ...
      // Parse JSON response
      const subtasks = JSON.parse(content) as string[];
...
export function createAIClient(): AIClient {
  if (AI_ENABLED && AI_API_KEY) {
    if (AI_PROVIDER === 'openai') {
      try {
        console.log(
          `Initializing OpenAI client with model: ${AI_MODEL}`
        );
        return new OpenAIClient(AI_API_KEY, AI_MODEL);
      } catch (error) {
        console.error(
          'Failed to create OpenAI client, falling back to rule-based:',
          error
        );
    ...
    console.warn(
      `Unknown AI provider: ${AI_PROVIDER}, using fallback implementation`
    );
  }
