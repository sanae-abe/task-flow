 ...
 */
export async function getRecommendedTaskWithAI(
  tasks: TaskRecord[],
  ...
  const scoreBreakdown = calculateDetailedScore(
    ...
    tasks,
    userContext,
    { considerTimeOfDay, considerHistory }
  );
 ...
 */
export async function getTopRecommendedTasks(
  tasks: TaskRecord[],
  userContext: UserContext,
  options: RecommendationOptions = {}
...
interface ScoreBreakdown {
  ...
  progressScore: number; // Week 8: Added progress-based scoring
  totalScore: number;
  confidence: number;
}
...
function calculateDetailedScore(
  ...
  allTasks: TaskRecord[],
  userContext: UserContext,
  options: { considerTimeOfDay?: boolean; considerHistory?: boolean }
): ScoreBreakdown {
  ...
  if (options.considerHistory && userContext.completionHistory) {
    breakdown.historyMatchScore = calculateHistoryMatchScore(
      task,
 ...
 */
function calculateHistoryMatchScore(
  _task: TaskRecord,
  ...
  if (similarCompletions.length === 0) return 0;

  // Calculate match score
  let score = 0;

  // Time-of-day match
  const timeMatches = similarCompletions.filter(
    item => Math.abs(item.timeOfDay - currentHour) <= 1
  );
  ...
  // Day-of-week match
  const dayMatches = similarCompletions.filter(
 ...
 */
function calculateWorkloadScore(allTasks: TaskRecord[]): number {
  let score = 0;
 ...
 */
function calculateProgressScore(task: TaskRecord): number {
  if (!task.subtasks || task.subtasks.length === 0) {
 ...
 */
export function analyzeWorkPatterns(history: CompletionHistoryItem[]): {
  peakHours: number[];
