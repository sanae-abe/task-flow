...
export async function breakdownTaskWithAI(
  ...
  const {
    strategy = determineOptimalStrategy(task),
    maxSubtasks = 7,
    minSubtasks = 2,
  } = options;
 ...
 */
export function determineOptimalStrategy(task: TaskRecord): BreakdownStrategy {
...
function generateReasoning(
  ...
  const strategyReasons: Record<BreakdownStrategy, string> = {
    SEQUENTIAL:
      'Tasks are broken down in sequential order for step-by-step execution',
 ...
 */
export function validateBreakdown(result: BreakdownResult): boolean {
 ...
 */
export function estimateSubtaskEffort(
  subtask: string,
