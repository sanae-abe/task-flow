...
export async function optimizeScheduleWithAI(
  ...
  if (balanceWorkload) {
    optimizedTasks = balanceWorkloadDistribution(
      optimizedTasks,
      constraints.workingHoursPerDay || 8
    );
  }
 ...
 */
function applyDependencyOrdering(tasks: TaskRecord[]): TaskRecord[] {
 ...
 */
function balanceWorkloadDistribution(
  tasks: TaskRecord[],
 ...
 */
function groupTasksByPriority(tasks: TaskRecord[]): {
  critical: TaskRecord[];
  high: TaskRecord[];
  medium: TaskRecord[];
  low: TaskRecord[];
  ...
  return tasks.reduce(
    (acc, task) => {
      const priority = task.priority.toLowerCase() as
        | 'critical'
        | 'high'
        | 'medium'
        | 'low';
    ...
    {
      critical: [] as TaskRecord[],
      high: [] as TaskRecord[],
      medium: [] as TaskRecord[],
      low: [] as TaskRecord[],
    }
  );
}
 ...
 */
export function estimateTaskEffort(task: TaskRecord): number {
...
function generateOptimizationSuggestions(
  ...
  if (overdueTasks.length > 0) {
    suggestions.push(
      `${overdueTasks.length} task(s) are overdue. Consider rescheduling or delegating.`
    );
  }
  ...
  if (constraints.deadline) {
    const totalEffort = tasks.reduce(
      (sum, t) => sum + estimateTaskEffort(t),
      0
    );
    const availableHours =
      Math.ceil(
        (constraints.deadline.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
      ) * (constraints.workingHoursPerDay || 8);

    if (totalEffort > availableHours) {
      suggestions.push(
        `Estimated ${totalEffort}h exceeds available ${availableHours}h. Consider extending deadline or reducing scope.`
      );
    }
  }
  ...
  if (grouped.critical.length > 10) {
    suggestions.push(
      `High number of critical tasks (${grouped.critical.length}). Consider re-evaluating priorities.`
    );
  }

  // Check for missing due dates
  const missingDueDates = tasks.filter(
    t => !t.dueDate && t.status !== 'COMPLETED'
  );
  if (missingDueDates.length > 5) {
    suggestions.push(
      `${missingDueDates.length} task(s) without due dates. Adding deadlines can improve planning.`
    );
  }
...
export function calculateWorkloadDistribution(
  ...
  for (let i = 0; i < days; i++) {
    ...
    const totalEffort = tasksForDay.reduce(
      (sum, t) => sum + estimateTaskEffort(t),
      0
    );
 ...
 */
export function findOptimalTaskOrder(
  tasks: TaskRecord[],
 ...
 */
export function identifyConflicts(tasks: TaskRecord[]): {
  overlappingDeadlines: TaskRecord[][];
  overloadedDays: Date[];
} {
  const conflicts = {
    overlappingDeadlines: [] as TaskRecord[][],
    overloadedDays: [] as Date[],
  };

  // Group tasks by due date
  const tasksByDate = new Map<number, TaskRecord[]>();
  tasks.forEach(task => {
    if (task.dueDate) {
  ...
  tasksByDate.forEach((tasksOnDate, dateKey) => {
    const totalEffort = tasksOnDate.reduce(
      (sum, t) => sum + estimateTaskEffort(t),
      0
    );
    if (totalEffort > 8) {
    ...
    const highPriorityTasks = tasksOnDate.filter(
