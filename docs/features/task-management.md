...
const sortByPriority = (tasks: Task[]) => {
  return tasks.sort((a, b) => PRIORITY_CONFIG[a.priority].order - PRIORITY_CONFIG[b.priority].order);
};
```
...
const filterTasks = (tasks: Task[], filters: TaskFilters) => {
  return tasks.filter(task => {
    ...
    if (filters.labels.length) {
      const hasMatchingLabel = task.labels.some(label => filters.labels.includes(label.id));
