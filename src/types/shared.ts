/**
 * 共通コンポーネントの型定義
 */

export type IconButtonVariant = 'default' | 'danger' | 'success' | 'warning' | 'muted';

export type IconButtonSize = 'small' | 'medium' | 'large';

export type IconButtonStyle = 'primer' | 'custom';

export type StatusBadgeVariant = 'default' | 'danger' | 'warning' | 'success' | 'info' | 'neutral' | 'emphasis';

export type StatusBadgeSize = 'small' | 'medium' | 'large';

export type FlexDirection = 'row' | 'row-reverse' | 'column' | 'column-reverse';

export type FlexAlign = 'start' | 'end' | 'center' | 'baseline' | 'stretch';

export type FlexJustify = 'start' | 'end' | 'center' | 'space-between' | 'space-around' | 'space-evenly';

export type FlexWrap = 'nowrap' | 'wrap' | 'wrap-reverse';

export type LoadingStateSize = 'small' | 'medium' | 'large';

export type LoadingStateVariant = 'default' | 'overlay' | 'inline';