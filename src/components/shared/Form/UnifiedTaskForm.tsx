...
export const UnifiedTaskForm = memo<UnifiedTaskFormProps>(
  ({
    ...
    const fields = useMemo(
      () => createTaskFormFields(fieldsConfig),
      [fieldsConfig]
    );
    ...
    return (
      <div className={`${baseClassName} ${className}`} onKeyDown={onKeyPress}>
        {/* テンプレート選択通知（作成モードのみ） */}
        {mode === 'create' &&
          createOptions?.showTemplateNotification &&
          createOptions.selectedTemplate && (
            <div className='mb-6'>
              <DialogFlashMessage
                message={{
                  type: 'info',
        ...
        {updatedFields.map(field => {
          ...
          return (
            <UnifiedFormField
                  ...
                  switch (field.name) {
                    ...
                    case 'selectedBoardId':
                      if (createOptions?.setSelectedBoardId) {
                        createOptions.setSelectedBoardId(
              ...
              touched={
                field.type !== 'custom'
                  ? form.state.touched[field.name]
                  : undefined
              }
              disabled={
                field.type !== 'custom' ? form.state.isSubmitting : undefined
              }
