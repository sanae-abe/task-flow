...
import { isValidKanbanBoard, isValidLabel, parseTypedJSON, Result, StorageError } from './type-guards';
...
const VALIDATORS: {
  ...
  [STORAGE_KEYS.USER_PREFERENCES]: (value): value is StorageSchema[typeof STORAGE_KEYS.USER_PREFERENCES] => {
  ...
  [STORAGE_KEYS.TABLE_COLUMNS]: (value): value is StorageSchema[typeof STORAGE_KEYS.TABLE_COLUMNS] => {
    if (typeof value !== 'object' || value === null) {return false;}
    const obj = value as Record<string, unknown>;
    return (
      Array.isArray(obj['visibleColumns']) &&
      (obj['visibleColumns'] as unknown[]).every(item => typeof item === 'string') &&
      typeof obj['columnWidths'] === 'object' &&
      obj['columnWidths'] !== null
    );
  },
  [STORAGE_KEYS.VIEW_MODE]: (value): value is StorageSchema[typeof STORAGE_KEYS.VIEW_MODE] => 
...
class TypedLocalStorage {
  ...
  getItem<K extends keyof StorageSchema>(
    ...
    try {
      const rawValue = localStorage.getItem(STORAGE_KEYS[key as keyof typeof STORAGE_KEYS]);
      if (rawValue === null) {
        return { success: false, error: new StorageError('Item not found', String(key), 'get') };
      }

      const parsedItem = parseTypedJSON(rawValue, (value): value is TypedStorageItem<StorageSchema[K]> => {
        if (typeof value !== 'object' || value === null) {return false;}
        const obj = value as Record<string, unknown>;
        return (
          'value' in obj &&
      ...
      if (!parsedItem) {
        return { success: false, error: new StorageError('Invalid JSON format', key, 'get') };
      }
      ...
      if (parsedItem.checksum) {
        ...
        if (generateChecksum(dataString) !== parsedItem.checksum) {
          return { success: false, error: new StorageError('Checksum mismatch', key, 'get') };
        }
      }
      ...
      if (!validatorFn(parsedItem.value)) {
        return { success: false, error: new StorageError('Type validation failed', key, 'get') };
      }
  ...
  setItem<K extends keyof StorageSchema>(
    ...
    try {
      ...
      if (!VALIDATORS[key](value)) {
        return { success: false, error: new StorageError('Type validation failed', key, 'set') };
      }
      ...
      localStorage.setItem(STORAGE_KEYS[key as keyof typeof STORAGE_KEYS], JSON.stringify(storageItem));
  ...
  // アイテム削除
  removeItem<K extends keyof StorageSchema>(key: K): Result<void, StorageError> {
    try {
  ...
  hasItem<K extends keyof StorageSchema>(key: K): boolean {
    return localStorage.getItem(STORAGE_KEYS[key as keyof typeof STORAGE_KEYS]) !== null;
  }
