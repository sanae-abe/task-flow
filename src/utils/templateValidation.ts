...
export const isValidCategory = (category: string): category is TemplateCategory => VALID_CATEGORIES.includes(category as TemplateCategory);
 ...
 * テンプレート説明のバリデーション
 */
export const validateTemplateDescription = (description: string): ValidationError | null => {
 ...
 * タスク説明のバリデーション
 */
export const validateTaskDescription = (description: string): ValidationError | null => {
 ...
 * 期限のバリデーション
 */
export const validateDueDate = (dueDate: string | null): ValidationError | null => {
 ...
 */
export const validateTemplateFormData = (formData: Partial<TemplateFormData>): ValidationResult => {
  ...
  // テンプレート説明
  const descriptionError = validateTemplateDescription(formData.description || '');
  ...
  if (taskTitleError) {errors.push(taskTitleError);}

  // タスク説明
  const taskDescriptionError = validateTaskDescription(formData.taskDescription || '');
 ...
 * テンプレート全体のバリデーション
 */
export const validateTemplate = (template: Partial<TaskTemplate>): ValidationResult => {
  const errors: ValidationError[] = [];
  ...
  if (typeof template.usageCount !== 'number' || template.usageCount < 0) {
    errors.push({ field: 'usageCount', message: '使用回数は0以上の数値である必要があります' });
  }

  // isFavorite
  if (typeof template.isFavorite !== 'boolean') {
    errors.push({ field: 'isFavorite', message: 'お気に入りはboolean型である必要があります' });
  }
 ...
 * 不正な値を安全な値に変換
 */
export const sanitizeTemplateFormData = (formData: Partial<TemplateFormData>): TemplateFormData => ({
 ...
 * 複数のテンプレートをバリデーション
 */
export const validateTemplates = (templates: Partial<TaskTemplate>[]): {
  ...
  const invalidTemplates: Array<{ template: Partial<TaskTemplate>; errors: ValidationError[] }> = [];
...
export const validateImportData = (data: unknown): {
  ...
  // 各テンプレートをバリデーション
  const { validTemplates, invalidTemplates } = validateTemplates(importData['templates'] as Partial<TaskTemplate>[]);
