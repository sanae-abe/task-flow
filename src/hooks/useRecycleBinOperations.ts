 ...
 */
export const useRecycleBinOperations = (
  onMessage?: MessageCallback
): UseRecycleBinOperationsResult => {
  ...
  const permanentlyDeleteTaskOperation = useCallback(
    async (taskId: string, taskTitle?: string) => {
      setDeletingTaskId(taskId);
      try {
        const { updatedBoards, success } = permanentlyDeleteTask(
          state.boards,
          taskId
        );
        if (success) {
  ...
  const emptyRecycleBinOperation = useCallback(async () => {
    ...
    try {
      ...
      onMessage?.({
        type: 'success',
        text: `${deletedCount}件のタスクを完全削除しました`,
      });
