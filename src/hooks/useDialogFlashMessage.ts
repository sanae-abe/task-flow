...
import type {
  ...
  DialogMessageType,
} from '../components/shared/DialogFlashMessage';
...
export const useDialogFlashMessage = (autoClearDelay: number = 2000) => {
  ...
  const showMessage = useCallback(
    (type: DialogMessageType, text: string, title?: string, delay?: number) => {
  ...
  const showMessageFromObject = useCallback(
    (messageObj: DialogFlashMessageData, delay?: number) => {
  ...
  const showSuccess = useCallback(
    (text: string, title?: string, delay?: number) => {
