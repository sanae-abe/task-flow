...
export const kanbanReducer = (state: KanbanState, action: KanbanAction): KanbanState => {
  ...
  // Board actions
  if (['SET_BOARDS', 'CREATE_BOARD', 'SWITCH_BOARD', 'UPDATE_BOARD', 'DELETE_BOARD'].includes(action.type)) {
    return handleBoardActions(state, action);
  }

  // Column actions
  if (['ADD_COLUMN', 'UPDATE_COLUMN', 'DELETE_COLUMN', 'REORDER_COLUMNS'].includes(action.type)) {
    return handleColumnActions(state, action);
  }

  // Task actions
  if (['ADD_TASK', 'UPDATE_TASK', 'DELETE_TASK', 'MOVE_TASK'].includes(action.type)) {
