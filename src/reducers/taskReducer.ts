...
export const handleTaskActions = (state: KanbanState, action: KanbanAction): KanbanState => {
  switch (action.type) {
    case 'ADD_TASK': {
      ...
      const { columnId, title, description, dueDate, priority = 'medium', labels = [], files = [] } = action.payload;
      const newTask: Task = {
        id: uuidv4(),
        title,
    ...
    case 'MOVE_TASK': {
      ...
      // 最右端のカラムかどうかを判定
      const isTargetRightmost = targetColumnId === state.currentBoard.columns[state.currentBoard.columns.length - 1]?.id;
      ...
      const updatedColumns = state.currentBoard.columns.map(column => {
        ...
        if (column.id === sourceColumnId && column.id === targetColumnId) {
          ...
          if (taskIndex !== -1) {
            ...
            newTasks.splice(safeTargetIndex, 0, { ...movedTask, ...updatedTask });
