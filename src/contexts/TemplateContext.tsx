...
import { TemplateStorage, TemplateStorageError } from '../utils/templateStorage';
...
type TemplateAction =
  ...
  | { type: 'IMPORT_TEMPLATES'; payload: { templates: TaskTemplate[]; merge: boolean } };
 ...
 * テンプレートのフィルタリング
 */
const filterTemplates = (templates: TaskTemplate[], filter: TemplateFilter): TaskTemplate[] => {
 ...
 * テンプレートのソート
 */
const sortTemplates = (templates: TaskTemplate[], sort: TemplateSortConfig): TaskTemplate[] => {
...
export const TemplateProvider: React.FC<TemplateProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const loadTemplates = async () => {
      ...
      try {
        ...
        const errorMessage = error instanceof TemplateStorageError
          ? error.message
          : 'テンプレートの読み込みに失敗しました';
   ...
   * テンプレートを作成
   */
  const createTemplate = useCallback(async (formData: TemplateFormData): Promise<TaskTemplate> => {
    try {
  ...
  const updateTemplate = useCallback(async (
    ...
    try {
      ...
      dispatch({ type: 'UPDATE_TEMPLATE', payload: { id, template: updatedTemplate } });
  ...
  const deleteTemplate = useCallback(async (id: string): Promise<boolean> => {
    try {
      ...
      if (success) {
        ...
        notify.success(`テンプレート「${template?.name || ''}」を削除しました`);
      } else {
        notify.error('テンプレートが見つかりません');
      }
   ...
   * テンプレートをエクスポート
   */
  const exportTemplates = useCallback((): TaskTemplate[] => state.templates, [state.templates]);

  /**
   * IDでテンプレートを取得
   */
  const getTemplateById = useCallback((id: string): TaskTemplate | undefined => state.templates.find(t => t.id === id), [state.templates]);

  /**
   * ストレージ情報を取得
   */
  const getStorageInfo = useCallback(() => TemplateStorage.getStorageInfo(), []);
