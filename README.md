...
- **自動削除システム**: 完了タスクの自動削除・保持期間設定・通知機能
- **ソフトデリート機能**: 削除前の一時保存・復元可能なごみ箱機能
- **テンプレート管理**: テンプレートのインポート・エクスポート・バックアップ機能

### ⚙️ 設定機能
- **デフォルトカラム設定**: 新しいボード作成時のカラム設定をカスタマイズ
- **自動削除設定**: 完了タスクの保持期間・通知タイミング・ソフトデリート設定
- **ごみ箱機能**: ソフトデリートされたタスクの管理・復元機能
...
│   ├── AutoDeletion/          # 自動削除システム（13コンポーネント）
...
│   ├── taskDeletion.ts       # タスク削除処理
│   └── deletionScheduler.ts  # 削除スケジューリング
└── App.tsx                    # メインアプリケーション
```
